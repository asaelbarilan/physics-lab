...
export default function Diagram({ settings: s, display: d, step, selected, onSelect, onHeightChange }: { settings: Settings; display: Display; step: number; selected: number; onSelect: (index: number) => void; onHeightChange: (height:number) => void }) {
  const drag = useRef<{id:number;y:number;height:number} | null>(null);
  const svgY = (event: ReactPointerEvent<SVGSVGElement>) => {
    const matrix = event.currentTarget.getScreenCTM();
    return matrix ? new DOMPoint(event.clientX,event.clientY).matrixTransform(matrix.inverse()).y : null;
  };
  const startDrag = (event: ReactPointerEvent<SVGSVGElement>) => {
    if(event.button !== 0 || !(event.target instanceof Element) || !event.target.closest('[data-drag-height]'))return;
    const y=svgY(event); if(y===null)return;
    event.preventDefault();
    drag.current={id:event.pointerId,y,height:s.height};
    event.currentTarget.setPointerCapture(event.pointerId);
  };
  const moveDrag = (event: ReactPointerEvent<SVGSVGElement>) => {
    const active=drag.current; if(!active || active.id!==event.pointerId)return;
    const y=svgY(event); if(y===null)return;
    onHeightChange(Math.max(80,Math.min(200,Math.round(active.height+(active.y-y)/1.55))));
  };
  ...
  return <svg onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onLostPointerCapture={()=>{drag.current=null}} className="diagram" viewBox="0 0 840 435" role="img" aria-labelledby="diagram-title diagram-desc"><title id="diagram-title">מסלול האור במראה מישורית</title><desc id="diagram-desc">עצם משמאל, מראה במרכז ודמות מדומה מימין. קרניים אדומות פוגעות במראה וכחולות מוחזרות לעין. קווים מקווקווים הם המשכים גיאומטריים בלבד.</desc><defs><pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".8" fill="#d6e2e8"/></pattern>{['incoming','outgoing'].map((id,i)=><marker key={id} id={id} viewBox="0 0 10 10" refX="10" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M 0 0 L 10 5 L 0 10 z" fill={i ? '#2273cb':'#db514e'}/></marker>)}</defs><rect width="840" height="435" fill="url(#grid)"/><rect x="424" width="416" height="385" fill="#e9f3f7" opacity=".55"/><text x="180" y="28" className="region-label">לפני המראה · אור אמיתי</text><text x="640" y="28" className="region-label">מאחורי המראה · המשך גיאומטרי</text><line x1="38" y1="366" x2="804" y2="366" stroke="#cbd9dd"/>
    ...
    {paths.map((p,i)=><g key={p.name}><g data-drag-height="" role="button" tabIndex={0} aria-label={`בחירת ${p.name}`} onClick={()=>onSelect(i)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(i)}}} className="point-button"><circle cx={X(p.source.x)} cy={Y(p.source.y)} r="12" fill="transparent"/><circle cx={X(p.source.x)} cy={Y(p.source.y)} r={i===selected?6:4} fill={i===selected?'#087d7c':'#fff'} stroke="#087d7c" strokeWidth="2"/></g>{d.image && step>=6 && <circle cx={X(p.image.x)} cy={Y(p.image.y)} r="4" fill="#83aeb3"/>}</g>)}
    <g transform={`translate(${X(-s.eyeDistance)},${Y(s.eyeHeight)})`}><ellipse rx="15" ry="9" fill="white" stroke="#203d51" strokeWidth="2"/><circle r="5" fill="#203d51"/><text y="-21" className="svg-label">עין</text></g><text x={X(-s.distance)} y="391" className="svg-label">עצם</text>{d.image&&step>=6&&<text x={X(s.distance)} y="391" className="svg-label">דמות מדומה</text>}
